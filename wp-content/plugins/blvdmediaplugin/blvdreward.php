	...
	if ( !is_admin() ) {
		if ($blvdmedia_status == "enabled") {
			if ($blvdmedia_load  == "loadindividual") {
				...
				$blvd_html_var .= '<param name="mode" value="transparent" />';
				$blvd_html_var .= '<param name="allowScriptAccess" value="always" />';
				$blvd_html_var .= '<div id="atOverlay" style="width:100%;height:100%;position:fixed;filter:alpha(opacity=80);-moz-opacity:80%;opacity:.8;">';
				...
				if (is_singular()) {
					...
					if ($string_search == true) {
						if(isset($_GET['pubid'])) { 
							...
							add_action('get_footer','blvd_add_js_css');
...
opacity:.4; !important; 
			...
			if(isset($_GET['pubid'])) { 
				if ($blvdmedia_load  == "loadall") {
					...
					<param name="mode" value="transparent" />
					<param name="allowScriptAccess" value="always" />
					<div id="atOverlay" style="width:100%;height:100%;background-color:Black;position:fixed;filter:alpha(opacity=80);-moz-opacity:80%;opacity:.8;">
